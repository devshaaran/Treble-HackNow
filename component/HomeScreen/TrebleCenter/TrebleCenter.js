...
export class YourTreble extends Component{
    ...
    componentDidMount(){
        axios.get(`https://treble-backend.herokuapp.com/treble/user/${this.context.state.phNumber}/`)
              .then((response) => {
                  console.log("happening");
                  this.setState({ elements : response.data.map((treble,index) => {
                      return <GreenBox name={treble.title} info={treble} key={index} navigation={this.props.navigation}/>
                  })})
              })
              .catch((error) => {
    ...
    return (
        ...
        <View style={styles.trebleBar}>
            ...
            <TouchableOpacity style={{marginLeft: 30}} onPress={() => this.props.navigation.navigate("AllTreble")}>
            <Text style={styles.passive}>Join Treble</Text>
            </TouchableOpacity>
        </View>
        <View style={styles.box}>
            <Image style={styles.music} source={images.TrebleCenter.illustration}/>
            <GreenButton name="Start a New Treble" navigation={this.props.navigation}/>
